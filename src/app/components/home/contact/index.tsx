...
const reset = () => {
  ...
  const handleSubmit = async (e: any) => {
    ...
    // fetch("https://formsubmit.co/ajax/aashishkumarsaini2003@gmail.com", 
    fetch("https://formsubmit.co/ajax/284e9afebc4498846627702e83d76a20", 
{
      method: "POST",
                ...
                {contactData?.socialLinks?.map((value: any, index: any) => {
                  return (
                    <div key={index}>
                      ...
                      >
                        {value?.title}
                      </Link> Preventing the invocation of real url*/}
                      <Link
  href={value?.href}
  target="_blank"
                ...
                {contactData?.contactInfo?.map((value: any, index: any) => {
                  return (
                    <div key={index}>
                      ...
                      >
                        {value?.label}
                      </Link> preveting the invocation of real urls*/}
                      <Link
                      href={ value?.link}
                      className="text-base lg:text-lg text-black font-normal border-b border-black pb-3 hover:text-primary hover:border-primary"
                    >
